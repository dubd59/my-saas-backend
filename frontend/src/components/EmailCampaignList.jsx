import React, { useState, useEffect } from 'react';

const EmailCampaignList = () => {
  const [emailCampaigns, setEmailCampaigns] = useState([]);

  useEffect(() => {
    const fetchEmailCampaigns = async () => {
      try {
        const response = await fetch('/email-campaigns/');
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        setEmailCampaigns(data);
      } catch (error) {
        console.error('Failed to fetch email campaigns:', error);
      }
    };

    fetchEmailCampaigns();
  }, []);

  return (
    <div style={{ border: '1px solid #ddd', padding: '20px', marginBottom: '20px', backgroundColor: '#fff', borderRadius: '5px' }}>
      <h2 style={{ color: '#38beba', marginBottom: '20px', textAlign: 'center' }}>Email Campaigns</h2>
      <ul style={{ listStyle: 'none', padding: 0 }}>
          {emailCampaigns.map((emailCampaign) => (
              <li key={emailCampaign.id} style={{ marginBottom: '10px', borderBottom: '1px solid #eee', paddingBottom: '5px' }}>
                  {emailCampaign.subject} - {emailCampaign.creator_id}
              </li>
          ))}
      </ul>
  </div>
  );
};

export default EmailCampaignList;
